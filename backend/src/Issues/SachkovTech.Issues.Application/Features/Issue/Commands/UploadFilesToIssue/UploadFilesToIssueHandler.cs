...
public class UploadFilesToIssueHandler : ICommandHandler<UploadFilesResponse, UploadFilesToIssueCommand>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var issueResult = await _issueRepository.GetById(issueId, cancellationToken);
        if (issueResult.IsFailure)
            return issueResult.Error.ToErrorList();

        var request = new UploadFilesRequest(nameof(Domain.Issue.Issue), command.IssueId, command.Files);
